...
if [ $htk_save == false ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_bnfeats_htk.JOB.log \
    ...
    copy-feats-to-htk --output-dir=$bnfeadir --output-ext=bnf ark:- || exit 1
  # Make list of htk features,
  find $bnfeadir -name *.bnf >$data/htkfeats.scp
  find $bnfeadir -name *.bnf >$bnfeadir/htkfeats.scp
  # Check sentence counts,
  N0=$(cat $srcdata/feats.scp | wc -l)
  N1=$(cat $data/htkfeats.scp | wc -l)
  #N1=$(find $data/htkfeats.scp | wc -l) # previously
