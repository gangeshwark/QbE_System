...
mkdir -p $dir/log
assume_srcdir=$(dirname $dir)
if [ ! -f $assume_srcdir/final.mdl ] || [ ! -f $assume_srcdir/splice_opts ] ; then
  abs_srcdir=$(cd $srcdir; pwd)
  (cd $assume_srcdir; 
    for f in  splice_opts cmvn_opts delta_opts final.mdl final.alimdl; do
      [ -e $abs_srcdir/$f ] && ln -s $abs_srcdir/$f
    done
  )
fi
...
if [ -z "$si_dir" ]; then # we need to do the speaker-independent decoding pass.
  ...
  if [ $stage -le 0 ]; then
    ...
    steps/decode.sh --srcdir $srcdir  --parallel-opts "$parallel_opts" --scoring-opts "$scoring_opts" \
